...
class active_model_params(sa.shared_assumptions):
    def __init__(self,which_GHG):
        ...
        self.d_h_limits = (5,1000) # Hydraulic diameter limits in mm
        ...
        self.LCOE = 0.02
        self.fan_and_shell_cost_per_m2 = 3700
        self.packing_cost_per_m3 = 250
        ...
        # Here we assume the reaction rate is limited by the mass transfer rate thru the boundary layer,
        # and that the reaction rate at the surface can be increased to match it by intensifying the light
        self.get_surface_activity = get_mass_transfer_flux_limit_to_surface
        # Pressure drop should be f(velocity, hydraulic diameter, depth, params)
        self.get_pressure_drop = monolith_pressure_drop
        
        # Values for LED
        self.LED_cost_per_kwh = 0.063 #Divide cost per kW by lifetime in hours
...
def calculate_active_amr_cost(params):
    ...
    n = 50 #Reduce this value to speed the solver up a lot; use at least 50 for final results.
    #n = n+np.random.randint(10)
    #Uncomment the above line to test optimizer stability by changing the grid points the brute force optimizer tests
